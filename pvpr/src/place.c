...
/************** Defines and static variables local to place.c *****************/
...
/* This routine controls the overall placement and routing of a circuit. */
 ...
 struct s_trace **empty_routing;
 char msg[BUFSIZE];
 int success, width_fac;
...
/* Binary search over channel width required? */
 ...
 if (router_opts.fixed_channel_width == NO_FIXED_CHANNEL_WIDTH) {
   ...
   /* Allocate the major routing structures. */
    ...
    empty_routing = alloc_route_structs();  /* Empty_routing initialized  *
                                             * to empty.                  */
 ...
 if (success == 0) {
 /*   restore_routing (empty_routing);*/  /* Erase the failed routing. */
 ...
 /*   routing_stats (full_stats, router_opts.route_type, 
           det_routing_arch.num_switch, det_routing_arch.num_segment,
 ...
 free_route_structs (empty_routing);
 fflush (stdout);
}
...
#ifdef SPEC_CPU2000          /* Reduce variation in final solution due to round off */
    fnum = my_frand();
#endif
...
#ifdef SPEC_CPU2000
 fprintf(cost_fp, "Cost recomputed from scratch is %f.\n", cost_check);
#else
