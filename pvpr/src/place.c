 ...
 * significant optimization).                                         */
 ...
 for (i=0; i<num_threads; i++) {
	 alloc_context(&(contexts[i]), update_freq, inverse_prev_bb_cost, inverse_prev_timing_cost, &annealing_sched, &placer_opts, net_cost, temp_net_cost, cost, bb_cost, timing_cost, delay_cost, rlim, duplicate_pins, unique_pin_list, d_max);
 }
 
 barrier_init(&barrier, num_threads);
 for (i=1; i<num_threads; i++) {
   pthread_create(&threads[i], NULL, &parallel_place, (void *) &(contexts[i]));
 }
...
void update_t (float *t, float std_dev, float rlim, 
...
float get_net_cost (int inet, struct s_bb *bbptr) {
