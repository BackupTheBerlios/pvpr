...
int main (int argc, char *argv[]) {
...
#ifndef SPEC_CPU2000
 char title[] = "\n\nVPR FPGA Placement and Routing Program Version 4.00 "
                "(Beta 3) by V. Betz.\n"
 ...
 struct s_segment_inf *segment_inf;
...
#ifdef SPEC_CPU2000
 cost_fp = my_fopen ("costs.out","a",0);
#endif
 printf("%s",title);
 ...
 * optional.                                                       */
   ...
   if (argc < 5) {
     ...
     printf("\t[-nx <int>] [-ny <int>] [-full_stats] [-fast]\n");
     printf("\t[-timing_analysis on | off]\n");
     ...
     printf("\t[-verify_binary_search] [-route_chan_width <int>]\n");
     printf("\n");
     exit(1);
   }
